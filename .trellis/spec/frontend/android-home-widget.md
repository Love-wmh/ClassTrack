...
- **In-app 「添加到桌面」 (added 2026-09-20)**: the schedule page's top bar carries a `CalendarPlus` icon button (rendered **only** when `isNativeWidgetSnapshotAvailable()` is true, i.e. Android native — in a browser there is no home screen to add to and the button would be a dead end). It opens a `Sheet` listing `WIDGET_PIN_PRESETS` (`app/features/schedule/widgetPinPresets.ts`): five presets, each = **target cell + layout style + 大格子表现**, with the honest note that the final size is decided by the launcher. The preset **ids must equal `WidgetPreset.java`'s whitelist** — native silently falls back to "no preset" on an unknown string, so `widgetPinPresets.test.ts` re-states the same literals and fails on drift. Native `requestPinWidget({preset})` validates the id (unknown → `INVALID_PAYLOAD`), checks `AppWidgetManager.isRequestPinAppWidgetSupported()`, stores the preset in a pending slot, and calls `requestPinAppWidget(provider, extras, null)`. It resolves `{supported, requested}` — **no error codes**: "unsupported" and "user cancelled the system sheet" are normal outcomes the UI explains (`WIDGET_PIN_MANUAL_HINT` / `WIDGET_PIN_CANCELLED_HINT`).
- **The pin size hint is smoke, not a contract** (device-measured 2026-09-20): `extras` carries `OPTION_APPWIDGET_MIN_WIDTH/HEIGHT` of the preset's target cell, but **AOSP Launcher3 ignored them** — the pin sheet labelled the widget 「4 × 3」 (the provider's `targetCellWidth/Height`) and placed a 4×3 instance for a 6×3 preset. So the copy promises only what we control: the style/表现 are applied, the size is "以桌面实际放下为准，可以自己拖动调整". Never word the UI as "will be placed at 6×3" until a specific launcher is measured to honour the hint.
