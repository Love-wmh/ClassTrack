...
- **The pending preset is consumed by whoever gets there first, and the render path must not skip it** (added 2026-09-20): most launchers open `WidgetConfigActivity` after placement (that path reads the slot and pre-selects the preset, plus shows the 「推荐 N×M 格子」 hint), but **Launcher3's pin flow placed the widget straight to defaults without opening the config page** — device-measured, `preset_applied` never fired from the Activity. `ClassTrackWidget.provideGlance` therefore also applies it, under three rules: (a) **only to an instance that has never been configured** (`WidgetStyleState.isConfigured` = none of the three keys written — "reads back as default" is *not* a valid test, an old instance falling back to defaults looks identical); (b) **atomically** via `WidgetPendingPreset.claim()`, never peek-then-consume — concurrent renders of two instances otherwise both apply the same preset (observed as two `preset_applied` lines in logcat); (c) the slot is one-shot with a 5-minute timeout, so a cancelled system sheet cannot leak into a later instance. Never apply a preset to an already-configured instance, and never let a failure to write leave the slot claimed by an instance that did not receive it.
