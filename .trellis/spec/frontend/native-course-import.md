...
- The shell WebView loads the packaged React asset at `https://appassets.androidplatform.net/?native-shell=1` through `WebViewAssetLoader`. The boot path must be the origin root so the client router matches its index route; a static file path such as `/index.html` matches no route and makes `root.tsx`'s `ErrorBoundary` replace the whole tree with a 404 page, which removes the shell bridge and the content slot. `WebViewAssetLoader` does not resolve directory indexes, so the shell path handler must map an empty or `/`-terminated suffix to `index.html`. The shell must not use `file://`, `loadData`, arbitrary remote URLs, or a Java copy of shadcn markup.
...
- Android shell delivery must run `pnpm cap:sync:android` before the debug build and `pnpm android:check-assets` after it. The guard compares `build/client`, `android/app/src/main/assets/public`, and APK `assets/public` byte-for-byte, checks local shell references, prints only hashes/counts, and rejects stale or missing APK assets; do not install an APK that fails this check.
- The navigation allowlist is separate from capture validation and is an explicit host plus path-prefix pair list, so a prefix granted for one host is never implicitly granted to another. For the current adapter it allows only HTTPS, default/443 port, no userinfo, and exactly these pairs: `jwxt.tjut.edu.cn` + `/jwapp/sys/wdkb`, `jwxt.tjut.edu.cn` + `/authserver`, `authserver.tjut.edu.cn` + `/authserver`. The CAS login host is a different host from the JinZhi host, confirmed by redacted device evidence; it must never be reachable for `/jwapp/sys/wdkb`. Unknown paths, HTTP, `intent:`, `file:`, `content:`, external hosts, non-default ports, and userinfo are blocked. A new CAS host/path may be added only after redacted device evidence and as an explicit entry; never use wildcard hosts, subdomain matching, or suffix matching.
...
- The Android asset guard test must cover local/inline shell references, rejection of remote references without echoing query or fragment data, and mismatch detection for stale/missing/extra APK entries.
- The shell boot path contract is covered at three layers because no single layer can see it whole: a Vitest route-match test proves the boot path still matches a client route (and that a file path such as `/index.html` does not), a JVM test proves the native URL constant and its predicates, and the Node asset guard proves the Java constant and the route table agree. Keep all three; dropping any one reopens the white-screen regression.
