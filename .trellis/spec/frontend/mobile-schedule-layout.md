...
| 教室 | 恒显（**无 `@` 前缀**：课名纯白加粗、教室小一号白色 85%，靠字号/字重/透明度分层区分；为空不渲染），不截断 | 同左 |
| 教师 | 由格子**实测内容高度**决定：放得下就整行显示（1x 手机的二连节格子通常放得下），放不下整行隐藏 | 同左 |
| 备注 | 同「教师」，且优先级最低：空间不够时第一个被丢弃 | 同左 |
| 格子视觉 | **平色**柔和中间调 + **纯白字**圆角卡片（`courseColors` 8 组平色：色相间隔 ≥30°——玫瑰350°/蜜橙20°/鹅黄49°/橄榄90°/翡翠160°/天空200°/堇紫250°/品红300°，HSB 饱和 ≤0.5、亮度 0.6–0.7）；无渐变、无文字阴影；非本周课 `bg-slate-300 opacity-75` 灰卡 + 「非本周」标记（默认不显示，可在个人中心开启） | 同左 |
