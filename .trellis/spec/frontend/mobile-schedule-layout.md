...
| 教师 | 格子实际高度 ≥7.5rem 时整行显示（容器查询），否则整行隐藏；2x full 档强制显示 | 阈值放宽为 ≥6.5rem，其余同左 |
| 备注 | 格子高度 ≥9rem 时整行显示，否则整行隐藏；2x full 档强制显示 | 同左 |
...
| 信息显隐优先级 | 课名 > `@教室` > 教师 > 单双周/「非本周」标签（<4rem 整行隐藏）> 备注；只整行隐藏，不裁半截 | 同左 |
