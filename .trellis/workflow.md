...
- 1.3 Configure context `[required · once]` — Claude Code, Cursor, OpenCode, Codex, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Oh My Pi, ZCode, Snow, Reasonix, Grok, Kimi Code (sub-agent-dispatch platforms only; inline platforms skip)。**Pi 在本项目已固化 inline，因此跳过此步。**
...
**本项目已固化 inline 模式：禁止派遣任何子代理，规划与实现全部由主会话直接完成（见 AGENTS.md「子代理禁令」）。**
...
**本项目已固化 inline 模式：禁止派遣任何子代理（`trellis-implement` / `trellis-check` / `trellis-research` 一律不调用），Phase 2 与 Phase 3 全部由主会话直接完成。见 AGENTS.md「子代理禁令」。**
Flow: 主会话直接编辑代码 -> 自检（`trellis-check` 技能）-> 全量门禁（`pnpm test` / `typecheck` / `lint` / `format:check`）-> `trellis-update-spec` -> commit（Phase 3.4）-> `/trellis:finish-work`。
Read context: `prd.md` -> `design.md if present` -> `implement.md if present`，以及相关 spec 与任务目录下的 `research/` 材料。
[/workflow-state:in_progress]
...
[codex-inline, Kilo, Antigravity, Devin, DeepSeek Harness, Pi]
