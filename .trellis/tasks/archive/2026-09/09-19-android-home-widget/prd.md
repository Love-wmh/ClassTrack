...
- **R8 每实例样式配置**：样式与「已上完的课」的处理方式按 **widget 实例**独立保存。放置时由系统启动配置页供选择；放置后可从小工具内的「样式」入口重新打开该实例的配置页修改。配置页必须能预览三种样式的实际长相；未配置过的实例使用默认值（**接下来** + 已上完灰显 —— 2026-09-20 由产品负责人从「全天课表」改为「接下来」，因为它在不额外配置的情况下同时回答了「现在上什么」和「今天还有什么」；选择器预览与该默认样式必须保持一致）。
