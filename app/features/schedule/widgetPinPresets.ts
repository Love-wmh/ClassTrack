...
export const WIDGET_PIN_PRESETS: readonly WidgetPinPreset[] = [
  {
    ...
    name: '接下来',
    cell: '3×2',
    description: '上面一张大卡片突出正在进行或接下来的一节课，下面接今天的课表；今天没课时卡片上写今天的日期与「今天无课」，课表改列明天。',
  ...
  },
  {
    ...
    name: '紧凑',
    cell: '1×2',
    description: '只放一张当前 / 接下来的课卡片，底部一行今天的节数；今天没课时显示今天的日期与一句没课提示。',
    hint: '桌面只剩一条窄位时用；信息最少，但一眼能看到下一节。',
  },
]

/**
 * 「尺寸可以自己调」这条**体验**提示（不是承诺）。
 *
 * **这是唯一的文案源**：加桌面板与导入后引导共用它，禁止任何一方再抄一份 ——
 * 两处各写一句必然漂移（本文件顶部那句「格子大小以桌面实际放下为准」就是这么改过一轮的）。
 *
 * 措辞只能说「可以自己调」：最终跨度由 launcher 决定（见契约「pin size hint is smoke, not a contract」），
 * 所以不能写成「调到某个格子就会怎样」。
 */
export const WIDGET_PIN_SIZE_TUNING_HINT = '放好之后还能拖动卡片改尺寸；同一档摆法多试几个尺寸，找到信息最舒服的那个。'
/**
 * 系统不支持一键添加时的说明。
 *
