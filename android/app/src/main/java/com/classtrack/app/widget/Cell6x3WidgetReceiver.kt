 ...
 *
 * 与其它 provider 的差异只有元数据（`res/xml/widget_info_6x3.xml`）与默认样式（见 `WidgetProviderRegistry`）；
 * 渲染逻辑完全共用 `ClassTrackWidget`。行为注意事项与 `ClassTrackWidgetReceiver` 一致：
 * **绝不在这里用 `goAsync()`**（见那个类的注释：实测会返回 null 并因 NPE 杀进程，表现为小工具空白）。
 */
class Cell6x3WidgetReceiver : GlanceAppWidgetReceiver() {
